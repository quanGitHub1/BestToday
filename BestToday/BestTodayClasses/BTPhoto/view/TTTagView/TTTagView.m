...
- (void)handlerTagButtonEvent:(TTCheckBoxButton *)sender {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(selectButtonWith:Title:)]) {
        [self.delegate selectButtonWith:self Title:sender.titleLabel.text];
    }
...
- (void)addTagToLast:(NSString *)tag {
    ...
    if (result.count == 0) {
        ...
        [_svContainer scrollRectToVisible:CGRectMake(_svContainer.left, _svContainer.contentSize.height - 50, kSCREEN_WIDTH, 50) animated:YES];
