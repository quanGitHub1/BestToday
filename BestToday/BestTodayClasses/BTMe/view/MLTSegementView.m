...
#define ItemWidth       FULL_WIDTH/2

#define ItemLineWidth    ScaleHeight(190)/2.0
...
@implementation MLTSegementView


- (UIColor *)titleColor{
    if (!_titleColor) {
        _titleColor = [UIColor colorWithHexString:@"#969696" alpha:1];
    }
    return _titleColor;
}
...
- (void)configSubLabel{
    ...
    //分割线
    _separateLine = [[UIView alloc]initWithFrame:CGRectMake(FULL_WIDTH/2, 0, 0.6, self.height)];
    
    _separateLine.backgroundColor = [UIColor colorWithHexString:@"#eeeeee"];

    
    //滚动条
    _scrollLine = [[UIView alloc]initWithFrame:CGRectMake(ScaleHeight(45), ItemHeight - self.scrollLineHeight, ItemLineWidth, self.scrollLineHeight)];
...
- (void)selectLabelWithIndex:(NSInteger)index{
    ...
    scrollLineFrame.origin.x = ItemWidth * index + (_offestX > 0 ? ScaleHeight(_offestX) : ScaleHeight(45));
