...
#import "LEHttpRequestManager.h"
#import "LECommonDefine.h"
#import "AFHTTPSessionManager.h"
//#import "MBProgressHUD+Add.h"

@implementation LEHttpRequestManager
...
+ (id)sharedInstance
{
    static LEHttpRequestManager * HttpRequest;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        HttpRequest = [[LEHttpRequestManager alloc] init];
    });
    return HttpRequest;
}
              ...
              showHUD:(UIView *)showView
{
    ...
    {
        [[LEHttpRequestManager sharedInstance] createUnloginedRequest:taskID WithParam:param withMethod:method success:success failure:failure showHUD:showView];
    }
}
...
- (void)createUnloginedRequest:(HTTP_COMMAND_LIST)taskID WithParam:(NSDictionary *)param withMethod:(HTTPRequestMethod)method success:(void(^)(id result))success failure:(void(^)(NSError *erro))failure showHUD:(UIView *)showView
{
     ...
     */
    if (showView) {
//        [SVProgressHUD showProgress:showView];
    }
...
//                [MBProgressHUD showError:[NSString stringWithFormat:@"code:%ld %@",error.code,error.localizedDescription] toView:window];
            ...
            if (success != nil)
            {
                ...
                if (showView) {
//                    [SVProgressHUD hideHUDForView:showView];
                }
...

{
    if (taskID < HTTP_METHOD_RESERVE)
    {
        [[LEHttpRequestManager sharedInstance] createUnloginedRequest:taskID WithParam:param withExParam:Exparam withMethod:method success:success failure:failure uploadFileProgress:uploadFileProgress];
    }
}
...
{
    if (URLString) {
        [[LEHttpRequestManager sharedInstance]createUnloginedDownloadFileWithURLString:URLString downloadFileProgress:downloadFileProgress setupFilePath:setupFilePath downloadCompletionHandler:downloadCompletionHandler];
    }
}
