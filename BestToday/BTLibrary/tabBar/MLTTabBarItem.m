...
- (void)setBadgeView:(MLTBadgeView *)badgeView
{
    if (_badgeView) {
...
- (void)updateImageViewFrame
{
    if (self.extendedIcon) {
        ...
        if (image) {
            [self.imageView setFrame:CGRectMake(self.imageView.frame.origin.x, self.imageView.frame.origin.y, 40, [self heightForExtendedIcon:image.size])];
        }
    }else {
